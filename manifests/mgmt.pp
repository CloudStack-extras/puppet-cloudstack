...
# Run cloud-setup-management script only if MySQL is installed and configured
#
# Requires:
#
# Package[ 'sudo' ]
...
class cloudstack::mgmt {
  ...
  exec { '/usr/bin/cloud-setup-management':
    ...
    require => [ Service[ 'cloud-management' ], Exec[ 'cloud_setup_databases' ] ],
  }
...
#    }
  ...
  exec { 'cloud_setup_databases':
    command => "/usr/bin/cloud-setup-databases cloud:dbpassword@localhost --deploy-as=root",
